...
one element at a time? Note that in the first step, when you consider an element with just the first element - that is 
already "sorted" since there's nothing to its left that is smaller.

In this challenge, don't print every time you move an element. Instead, print the array after each iteration of the 
insertion-sort, i.e., whenever the next element is placed at its correct position.
...
Next, 3 is inserted next to 1, and the array is printed out. This continues one element at a time 
    ...
    {       
        ...
        for(int i=0; i<ar.length; i++){
            if(i+1<ar.length && ar[i]>ar[i+1]){
                int x=i;
                while(x>=0 && ar[x]>ar[x+1] &&  x<ar.length){
                    int t= ar[x+1];
                    ar[x+1]=ar[x];
                    ar[x]=t;
                    x--;
